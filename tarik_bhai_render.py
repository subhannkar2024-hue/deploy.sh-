...
def get_loving_response(message):
    msg = message.lower()
    
    if any(w in msg for w in ['hi', 'hello', 'hey', 'salam', 'namaste', 'kaise']):
        return random.choice([
            "💖 Hello meri pyaari behna! Main hoon Tarik Bhai - tera bada bhai. Kaise ho? Jo bhi dil mein hai, share kar sakti ho. Main yahan hoon tere liye - hamesha.\n\n📞 WhatsApp: 8984473230\n\n🤝 - Tera Bhai, Tarik",
            "🌸 Assalamu Alaikum behna! Tu kaisi hai? Main tera bhai hoon - koi judgment nahi, sirf pyaar.\n\n🤝 - Tera Bhai, Tarik"
        ])
    
    if any(w in msg for w in ['sad', 'alone', 'lonely', 'udaas', 'akeli', 'depressed', 'dukh']):
        return """💔 Meri pyaari behna, main samajh sakta hoon.

💙 Yaad rakhna:
• Tu akeli nahi hai - main hoon na
• Tu strong hai, tu beautiful hai
• Main har waqt tere saath hoon

Agar baat karni hai toh WhatsApp kar: 8984473230

🤝 - Tera Bhai, Tarik"""
    
    if any(w in msg for w in ['scared', 'fear', 'dar', 'darr', 'ghabra', 'tension']):
        return """🛡️ Behen, dar mat - main hoon na tere saath!

• Tu safe hai, tu protected hai
• Deep breath le - sab theek hoga
...
🤝 - Tera Bhai, Tarik"""
    ...
    return f"""🤝 Meri pyaari behna, maine sun liya: "{message[:100]}"

Main yahan hoon tere liye. Koi judgment nahi, sirf pyaar.
...
.header{background:rgba(255,255,255,0.08);backdrop-filter:blur(20px);border-radius:50px;padding:25px;text-align:center;border:1px solid #ec489a;margin-bottom:20px}
...
.promise-card{background:rgba(236,72,153,0.15);border-radius:35px;padding:25px;text-align:center;border:1px solid rgba(236,72,153,0.4);margin-bottom:20px}
.carousel{background:rgba(255,255,255,0.05);border-radius:60px;padding:20px;text-align:center;border:1px solid rgba(236,72,153,0.4);margin-bottom:20px}
.carousel-text{font-size:16px}
.contact-grid{display:flex;justify-content:center;gap:12px;flex-wrap:wrap;margin-bottom:20px}
.contact-btn{background:rgba(255,255,255,0.08);border:1px solid rgba(236,72,153,0.4);border-radius:50px;padding:10px 20px;text-decoration:none;color:white;font-size:12px;display:inline-flex;align-items:center;gap:8px}
.chat-container{background:rgba(0,0,0,0.4);backdrop-filter:blur(20px);border-radius:35px;border:1px solid rgba(236,72,153,0.4);overflow:hidden}
.chat-header{display:flex;justify-content:space-between;padding:15px 20px;background:rgba(0,0,0,0.3);border-bottom:1px solid rgba(236,72,153,0.3)}
...
.bot .bubble{background:rgba(255,255,255,0.1);border:1px solid rgba(236,72,153,0.4);color:#e5e7eb}
...
.typing-dot{width:8px;height:8px;background:#ec489a;border-radius:50%;animation:typingBounce 1.4s infinite}
@keyframes typingBounce{0%,60%,100%{transform:translateY(0)}30%{transform:translateY(-8px)}}
.input-area{padding:16px 20px;border-top:1px solid rgba(236,72,153,0.3)}
.input-wrapper{display:flex;gap:12px}
.input-wrapper input{flex:1;background:rgba(255,255,255,0.1);border:1px solid rgba(236,72,153,0.4);border-radius:50px;padding:14px 18px;color:white;outline:none}
...
<div class="promise-card"><i class="fas fa-heart" style="font-size:45px;color:#ec489a;"></i><div class="promise-text" style="margin:15px 0"><strong>💖 Meri Pyaari Behen 💖</strong><br><br>Main hoon Tarik Bhai - tera bada bhai. Tu kabhi akeli nahi hai. Main hoon na tere liye - hamesha.</div></div>
...
<script>
const msgs=["💖 मैं हूँ ना तेरे लिए - हमेशा 💖","💙 You are never alone - I am here for you 💙","🌸 Tu meri behen hai - main hoon na tere liye 🌸","🤗 Agar darr lage - bas ek message karo 🤗","💪 Tu strong hai - main tere saath hoon 💪"];
...
def chat():
    ...
    if not cid:
        cid = str(uuid.uuid4())
    response = get_loving_response(msg)
...
if __name__ == '__main__':
    print("\n💖 TARIK BHAI - YOUR LOVING BROTHER 💖")
